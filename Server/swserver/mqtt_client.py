...
def main(json_file=None):
    ...
    if json_file is None:
        addr = os.path.join(os.path.dirname(__file__),'json','smartwatch_messages.json')
        print("Using task configuration from: ", addr)
        f = open(addr)
    else:
        print("Using task configuration from: ", json_file)
        f = open(json_file)
    sw_msgs = json.load(f)
